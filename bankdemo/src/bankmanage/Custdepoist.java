...
public class Custdepoist extends JFrame implements ActionListener{
	    private static final long serialVersionUID = 1L;
	    static Custdepoist cdp;
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(e.getSource() == trans){
				try {
					...
					JOptionPane.showMessageDialog(cdp, e1.getMessage());
				}
				if(id.equals("") || pwd.equals("") || mon.equals(""))
					JOptionPane.showMessageDialog(cdp, "账号、密码或存入金额为空！"); 
				else{
					try {
						...
						if(rs.next()){
							if(rs.getString("custpwd").equals(pwd)){
								...
								if(a1 == 1){
									JOptionPane.showMessageDialog(cdp, "存款成功");
									setVisible(false);
								}
								else{
									JOptionPane.showMessageDialog(cdp, "存款失败！");
									setVisible(true);
								}
								stmt1.close();
							}
